...
const PurchasedBooks = () => {
    ...
    const { data: purchasedBooks = [], isLoading } = useQuery({
        queryKey: ['purchasedBooks', user?.email],
        queryFn: async () => {
            const res = await axiosPublic.post('/UserpaidCart', { email }); // Send email in the body
            return res.data;
        }
    });
    ...
    return (
        <div className="bg-[#FFFFFF]">
            <DashboardTitle title="Purchase History"></DashboardTitle>
                            ...
                            <table className="w-full text-[14px]">
                                ...
                                <tbody>
                                    {purchasedBooks.map((book) => (
                                        <tr key={book._id} className="border-b border-opacity-20 border-gray-300">
                                            <td className="p-3">
                                                <img 
                                                    src={book.pic} 
                                                    alt={book.name} 
